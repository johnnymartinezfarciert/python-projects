...
############## suits.index(suit) should be names.index(name) ##########3
deck = [ (name + ' of ' + suit, names.index(name) ) for name in names for suit in suits ]
################ forgot to randomize selection ! #######################333
#added random.shuffle(deck)
random.shuffle(deck)
...
while deck:
 ...
 if cardA[1] == cardB[1]:
  ########### the bonus should increase not take on the total of A's score ! ########################
  # bonus += scoreA
  bonus += 1
  outcome = 'ties'
